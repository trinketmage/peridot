...
(function(mod) {
  ...
  var wordOperators = wordRegexp([
    messages[locale].operators.and,
    messages[locale].operators.or,
    "!"
  ]);
  var commonKeywords = [
    messages[locale].structures.define,
  ...
  var conditionKeywords = [
    messages[locale].conditions.wallFace,
  ...
  cm.defineMode("peridot", function(conf, parserConf) {
    ...
    function tokenBaseInner(stream, state) {
      ...
      if (stream.match(condition)) return "condition";

      //custom keywords
      if (stream.match(wordRegexp(move))) return "move";
      if (stream.match(wordRegexp(left))) return "left";
      if (stream.match(wordRegexp(drop))) return "droptoken";
      if (stream.match(wordRegexp(pick))) return "picktoken";
      ...
      if (stream.match(identifiers)) {
        if (
          state.lastToken == messages[locale].structures.define &&
          stream.peek() != null
        ) {
          if (
            newWords.length == 0 ||
            newWords.indexOf(stream.current()) == -1
          ) {
