...
////////////////////////////////////////////////////////////////////////
const move = [messages[locale].instructions.forward];
const left = "gauche";
...
export default function parseAll(array) {
  execute = [];
  var i = 0;
  ...
  while (i < array.length) {
    ...
    if (moveRegex.test(array[i])) {
      ...
      defineRegex.test(array[i]) &&
      array[i].match(defineRegex)[1] == defineInstruction
    ) {
      ...
      repeatStructureRegex.test(array[i]) &&
      ...
      elseStructureRegex.test(array[i]) &&
...
function parseIf(array, i, state, category) {
  ...
  var startLine = getStartLine(state);
  //update state
  var newState = { defLine: state.defLine, lvl: 1, parent: startLine + i };
  ...
  if (j == i + 1) {
    ...
    if (
      ...
      elseStructureRegex.test(array[j]) &&
...
function parseLoop(array, i, state) {
  ...
  if (j == i + 1) {
    throw "no instructions inside loop " +
      array[i].type +
      " " +
      array[i].category;
  }
...
function parseCondition(array) {
  ...
  if (array[leftStartIndex] == "(") {
    ...
    //array starts with single expression
  ...
  if (leftObject && rightObject) {
    ...
    return {
      ope: ope,
      left: leftObject,
      right: right ? parseCondition(right) : null
    };
  } else if (rightObject) {
    return {
      ope: ope,
      left: left ? parseCondition(left) : null,
      right: rightObject
    };
  }
  return {
